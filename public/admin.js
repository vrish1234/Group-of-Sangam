const API_BASE = (window.GYAN_SETU_CONFIG && window.GYAN_SETU_CONFIG.API_BASE
  ? String(window.GYAN_SETU_CONFIG.API_BASE).replace(/\/$/, '')
  : '');
...
async function loadStudents() {
  const response = await fetch(apiUrl(`/api/admin/students?page=${page}`));
...
bulkUploadBtn.addEventListener('click', async () => {
  const response = await fetch(apiUrl('/api/admin/bulk-assign'), {
    method: 'POST',
...
resultToggle.addEventListener('change', async () => {
  const response = await fetch(apiUrl('/api/admin/result-toggle'), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
